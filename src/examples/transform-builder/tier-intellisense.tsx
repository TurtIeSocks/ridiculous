  ...
  return (
    <div className="glass-card flex flex-col rounded-2xl p-6">
      ...
      <p className="mt-2 text-muted-foreground text-sm">
        State typed as <code className="text-foreground">TransformString</code>{" "}
        — the union of every{" "}
        <code className="text-foreground">translate()/scale()/rotate()/…</code>{" "}
        head plus <code className="text-foreground">none</code>. It is also the{" "}
      ...
      <pre className="mt-6 overflow-x-auto rounded-lg border border-white/10 bg-black/40 p-4 font-mono text-[11px] leading-relaxed">
        ...
        {"<"}
        <span className="text-pink-glow">TransformString</span>
        {">"}(<span className="text-emerald-400">&quot;scale(1.25)&quot;</span>)
      </pre>
    </div>
  )
}
